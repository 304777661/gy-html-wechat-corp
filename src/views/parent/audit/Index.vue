<template>
  <div class="audit">
    ...
    <van-cell-group v-if="audit.passList && audit.passList.length > 0" :border="true">
      <van-cell title="已通过部分" class="audit-sticky"></van-cell>
      <div v-for="(pass,index) in audit.passList" :key="'pass'+index" class="van-hairline--bottom">
        <van-cell :title="'原: '+$enums.ChangeType.getName(pass.changeType)" :value="pass.origValue"></van-cell>
        <van-cell :title="'新: '+$enums.ChangeType.getName(pass.changeType)" :value="pass.newValue"
                  :border="true"></van-cell>
      </div>
    </van-cell-group>

    <van-cell-group v-if="audit.notPassList && audit.notPassList.length > 0" :border="true">
      <van-cell title="未通过部分" class="audit-sticky"></van-cell>
      <div v-for="(pass,index) in audit.notPassList" :key="'pass'+index" class="van-hairline--bottom">
        <van-cell :title="'原: '+$enums.ChangeType.getName(pass.changeType)" :value="pass.origValue"></van-cell>
        <van-cell :title="'新: '+$enums.ChangeType.getName(pass.changeType)" :value="pass.newValue"
                  :border="true"></van-cell>
      </div>
